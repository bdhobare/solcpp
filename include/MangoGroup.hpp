...
struct MangoGroup {
  ...
  auto getSpotMarketIndex(const solana::PublicKey& spotMarketPk) {
    ...
    throw std::runtime_error(
        "This Market does not belong to this MangoGroup: " +
        spotMarketPk.toBase58());
  }
  auto getPerpMarketIndex(const solana::PublicKey& perpMarketPk) {
    ...
    throw std::runtime_error(
        "This PerpMarket does not belong to this MangoGroup: " +
  ...
  auto getTokenIndex(const solana::PublicKey& token) {
    ...
    throw std::runtime_error("This token does not belong in this MangoGroup: " +
                             token.toBase58());
  }
  auto getRootBankIndex(const solana::PublicKey& rootBank) {
    ...
    throw std::runtime_error(
        "This root bank does not belong in this MangoGroup: " +
        rootBank.toBase58());
  }
};
}  // namespace mango_v3
